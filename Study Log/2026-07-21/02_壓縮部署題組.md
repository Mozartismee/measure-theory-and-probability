...
# 壓縮部署題組 — Product Integration to Conditional Density

Use the same model across Exercises 2, 3 and 6 whenever possible. Reusing an object across representations is part of the task.

## Exercise 0 — Canonical Tonelli assignment

Complete Exercise A in the formal [Product Measures TD](../../35_Product-Measures-and-Transformations/02_TD/TD%2001%20—%20Product%20Measures,%20Iterated%20Integrals,%20and%20Transformations.md). Record the first failed step, if any, in the session record. This pointer is not an additional exercise.

## Exercise 1 — Canonical Fubini-failure assignment

Complete Exercise 0 in the formal Product Measures TD. Record both iterated sums, the absolute sum and the first failed theorem hypothesis. This pointer is not an additional exercise.
...
First complete Exercise 2(1)–(3) in the formal Product Measures TD. Apply those results to the density above.

1. Verify normalization and compute $f_X$ and $f_Z$ explicitly.
2. For every bounded Borel $\varphi$, identify the canonical identity that yields
...
3. Fill the ledger rows for $f_{X,Z}$ and $f_X$, including their different reference measures and a.e. bases.
...
## Exercise 4 — RN across three spaces
...
4. Add the probability-space and state-space rows to the density ledger.
...
## Exercise 8 — Official-anchor classification

Using the July 2026 official sample-question PDF, and without reproducing the statements, complete:
...
7. fill five rows of the density ledger from memory.
